...
void hook_timer()
{
  ...
  if (*NOW-last_save>=ASAVE_WAIT)
    {
      if (words->mchanged()) words->save();
      if (blacklist->mchanged()) blacklist->save();
      if (ignore->mchanged()) ignore->save();
...
extern "C" module *init()
{
  ...
  ignore=new Words(IGNORE);
  chans=new Chans(WCHANS);
  blacklist=new BlackList(BLIST);
...
extern "C" void destroy()
{
  ...
  if (words!=NULL)     { delete words;words=NULL; }
  if (ignore!=NULL)    { delete ignore;ignore=NULL; }
  if (chans!=NULL)     { delete chans;chans=NULL;  }
  if (blacklist!=NULL) { delete blacklist;blacklist=NULL; }
}
