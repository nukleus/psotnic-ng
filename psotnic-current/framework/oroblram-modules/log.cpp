...
/*
  ...
  Usage:
        Set the parameters below and then away you go :)
...
/*
  fileExists
   ...
   file  : the filename
   --
   returns : true if the file exists, false otherwise (or if the file isnt readable I think.. in which case ERRORCITY.)
*/
...
/*
  ...
  --
  channel  : the channel name to search for
  --
  returns  : true if the channel should be logged, otherwise false
*/
bool isIn(string channel) { 
  bool ret = false; // our return value
  // loop through our channel list and see if we can find the channel
  for (int i = 0;i < MAX_CHANNELS;i++) { if (LOG_CHANNELS[i] == channel) { ret = true; break; } }
  // return
  return ret;
}
...
/*
  ...
  channel : the channel to log it to
  --
  returns : nothing
*/
...
string getNick(const char *from) {
  ...
  if (ret.find("!") > 0) { ret = ret.substr(0,ret.find("!")); } 
  // return
  return ret;
}
